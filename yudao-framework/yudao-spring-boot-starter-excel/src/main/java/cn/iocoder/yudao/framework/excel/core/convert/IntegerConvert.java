...
public class IntegerConvert implements Converter<Integer> {
    ...
    private int getRowIndexFromReadCellData(ReadCellData readCellData) {
        ...
        log.debug("[IntegerConvert] 使用ConversionErrorHolder中的当前行号: {}", currentRowIndex);
        ...
        try {
            ...
            if (rowIndexObj instanceof Integer) {
                ...
                log.debug("[IntegerConvert] 从ReadCellData获取行号: {} -> {}", rowIndex, convertedRowIndex);
                return convertedRowIndex;
            }
        } catch (Exception e) {
            // 如果无法获取行号，记录日志
            log.debug("[IntegerConvert] 无法从ReadCellData获取行号: {}", e.getMessage());
        }

        // 如果都无法获取，返回1作为默认值（避免返回0）
        log.warn("[IntegerConvert] 无法获取行号，使用默认值1");
        return 1;
    }
